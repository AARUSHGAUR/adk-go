...
func TestRemoteAgent_E2E(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			remote, err := newTestClient(regSrv).RemoteAgent(context.Background(), "projects/p/locations/l/agents/echo")
			if err != nil {
...
func runAndCollectText(t *testing.T, ag agent.Agent, prompt string) string {
	...
	for ev, err := range r.Run(context.Background(), "user", "session", msg, agent.RunConfig{}) {
		if err != nil {
