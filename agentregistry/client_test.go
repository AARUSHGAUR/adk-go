...
func TestModelName(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "parses model resource name",
			respBody:  `{"interfaces":[{"url":"projects/p/locations/l/publishers/google/models/gemini-2.0:predict"}]}`,
			wantModel: "projects/p/locations/l/publishers/google/models/gemini-2.0",
		},
		{
			name:     "missing connection URI is an error",
			respBody: `{"displayName":"Gemini"}`,
			wantErr:  true,
		},
		{
			name:     "endpoint fetch failure is surfaced",
			respBody: `{"error":"boom"}`,
			status:   http.StatusServiceUnavailable,
			wantErr:  true,
		},
