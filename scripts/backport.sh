...
# filter on the *base* branch, and v1 is listed. When the PR is opened from CI,
# by github-actions[bot], the runs are created in an approval-required state and
# wait for a human to release them, which is what the message below is about.
#
# Not fatal. The branch is pushed and the PR is open by the time this runs, so
# failing the script here would report a backport that in fact succeeded. It is
# a prompt, not a gate.
confirm_checks() {
  local pr="$1"
  ...
  The backport itself is fine -- the branch is pushed and PR #${pr} is open --
  but it cannot be merged until CI reports.

  Most likely the runs are waiting on approval. A pull request opened by
  github-actions[bot] gets its workflows in an approval-required state: open the
  PR and click "Approve workflows to run" in the merge box. Anyone with write
  access can do it.

  If there is no such banner either, re-fire the pull_request event with

    gh pr close ${pr} --repo ${REPO} && gh pr reopen ${pr} --repo ${REPO}

  A 'reopened' event re-runs the workflows; pushing another commit works too.
EOF
  return 0
}
...
CI held, and this one cannot be merged until it reports.
